...
::: tip React vs Vue
React refs use `.current` to access the DOM element. Vue refs use `.value`.
:::
...
| `globalGetRefStackItem<T>(key)` | Get a single ref by key. Returns `RefObject<T>` (React) or `Ref<T>` (Vue), or `undefined`. |
